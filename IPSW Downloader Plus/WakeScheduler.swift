...
enum WakeScheduler {
    ...
    static func scheduleWake(hour: Int, minute: Int, days: Set<Int>) -> String? {
        // Calcola orario di sveglia: 2 minuti prima dell'avvio pianificato
        ...
        let timeString = String(format: "%02d:%02d:00", wakeHour, wakeMinute)

        // Costruisce la stringa giorni per pmset (es. "MTWRFSU" = ogni giorno)
        ...
        guard !dayString.isEmpty else { return "Nessun giorno valido selezionato." }

        let command = "/usr/bin/pmset repeat wakeorpoweron \(dayString) \(timeString)"
